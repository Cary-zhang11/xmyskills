...
## 编号规则（强制）
- 格式：`tc-p0-001`、`tc-p1-001`（`tc-pX-NNN`）
- 在同一 `### 二级功能块` 内按优先级独立递增（每个优先级从 001 起算）
- 不允许重复编号（同一 ### 块内同优先级不出现相同 NNN）
- core 始终使用 `tc-pX-NNN` 格式；full 开启模板对齐时，若模板为短编号 `tc-pX`，full 可跟随短编号，但 core 不变
